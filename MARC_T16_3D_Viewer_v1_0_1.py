...
FIX: Real element/node IDs from .t16 now shown in viewer (was showing array indices)
FIX: Value lookup accepts real IDs (E137304, N12345)
NEW: Linear extrapolation from integration points (matches Mentat default)
NEW: Legend Extrapolation dialog with Mentat-style method and nodal averaging options
CHANGE: Centroid option removed from Output Available; extrapolation now drives element-local contour display
...
GUIDELINE_PATH = r"\\frafil002\VC_FEA\VC-Marc_Post\Marc_Tools_Guideline\Marc_T16_3D_Viewer_User_Guide.html"
...
def build_export_cache_path(base_path):
    return base_path + "_3D_View.cache.pkl"
...
def save_export_cache(cache_path, data):
    try:
        with open(cache_path, "wb") as cf:
            pickle.dump(data, cf, protocol=pickle.HIGHEST_PROTOCOL)
        return True
    except Exception:
        return False
...
def get_global_cache_dir(source_file):
    try:
        cache_dir = os.path.join(os.path.dirname(os.path.abspath(source_file)), ".t16_3d_viewer_cache")
...
def build_geometry_cache_path(source_file, mesh_fingerprint):
    cache_dir = get_global_cache_dir(source_file)
    if not cache_dir:
        return None
    return os.path.join(cache_dir, "mesh_{}.geom.pkl".format(mesh_fingerprint))


def compute_mesh_fingerprint(reader):
    """Stable fingerprint from mesh topology/IDs for cross-file geometry cache."""
    h = hashlib.sha1()
    h.update(str(int(reader.n_nodes or 0)).encode("ascii"))
    h.update(b"|")
    h.update(str(int(reader.n_elements or 0)).encode("ascii"))
    h.update(b"|")

    try:
        if reader.node_ids is not None:
            node_ids = np.asarray(reader.node_ids, dtype=np.int64)
        else:
            node_ids = np.arange(1, int(reader.n_nodes or 0) + 1, dtype=np.int64)
        h.update(node_ids.tobytes())
    except Exception:
        pass

    for elem in reader.elements or []:
        try:
            eid = int(elem.get("id", 0))
            etype = int(elem.get("type", 0))
            conn = elem.get("connectivity_ids", []) or []
            h.update(eid.to_bytes(8, "little", signed=True))
            h.update(etype.to_bytes(4, "little", signed=True))
            h.update(len(conn).to_bytes(4, "little", signed=False))
            for nid in conn:
                h.update(int(nid).to_bytes(8, "little", signed=True))
        except Exception:
            continue
...
def pack_normalized_u16(norm_values):
    """Compact normalized float array [0..1] into uint16 list."""
    if norm_values is None:
        return None
    try:
        arr = np.nan_to_num(norm_values, nan=0.0, posinf=0.0, neginf=0.0)
        packed = np.clip(np.round(arr * 65535.0), 0, 65535).astype(np.uint16)
...
def ensure_runtime_dependencies():
    ...
    if post_open is None:
        from py_post import post_open as _post_open
...
HARMONIC_TITLE_RE = _re.compile(r'(?<!\d)(\d+)\s*:\s*(\d+)(?!\d)')
HARMONIC_TOKEN_RE = _re.compile(r'^(\\d+)\\s*:\\s*(\\d+)$')
...
# =============================================================================
# T16 READER - SAME DATA CONTRACT USED BY THE VIEWER
# =============================================================================
...
class T16Reader:
    def __init__(self, filepath):
        ...
        self.n_elements = 0
        self.debug_info = []
        self._all_increment_indices = []
        self._active_increment_indices = []
        self._vector_catalog = {}
        self._node_scalar_catalog = {}
        self._elem_scalar_catalog = {}
        self._var_info = {}
        self._var_locations = {}
        self._disp_vector_name = None
        self._loaded_outputs = set()
        self._time_denominator = 1.0
        self._increment_ids = {}
        self._increment_labels = {}
        self._static_increment_indices = []
        self._harmonic_increment_indices = []
        self._has_harmonic_increments = False
        self._harmonic_base_increment_id = None
        self._connectivity_index_mode = False
        self.material_names = []
        self.material_dat_path = None

    def _emit_progress(self, progress_callback, pct, msg):
        if progress_callback:
            try:
                progress_callback(float(max(0.0, min(100.0, pct))), msg)
            except:
                pass
    ...
    def open(self, progress_callback=None):
        ...
        self._emit_progress(progress_callback, 1.0, "Reading sidecar .dat material info...")
        try:
            self.material_names, self.material_dat_path = resolve_material_names_from_sidecar(self.filepath)
        except Exception:
            self.material_names, self.material_dat_path = [], None
        self._emit_progress(progress_callback, 2.0, "Loading runtime dependencies...")
        ensure_runtime_dependencies()
        if post_open is None:
            raise ImportError("py_post is not available. Marc Mentat Python API is required.")
        self._emit_progress(progress_callback, 3.0, "Opening .t16/.t19 file...")
        self.file = post_open(self.filepath)
        self._read_all(progress_callback=progress_callback)
        return self
    ...
    def _normalize_connectivity(self, elem_type, connectivity_ids):
        conn = [safe_number(v, -1, as_int=True) for v in connectivity_ids]
        conn = [c for c in conn if c >= 0]
        if not conn:
            return []

        if elem_type in (157, 134):
            return conn[:4]
        if elem_type in (84, 7):
            return conn[:8]
    ...
    def _ip_average(self, ip_values):
        try:
            if hasattr(ip_values, "__iter__") and not isinstance(ip_values, (str, bytes)):
                vals = []
                for s in ip_values:
                    if hasattr(s, "value"):
                ...
                return 0.0
            if hasattr(ip_values, "value"):
                return float(ip_values.value)
            return float(ip_values)
        except:
            return 0.0

    def _build_unique_state_key(self, increment_index):
        return "STATE_{}".format(increment_index)

    def _clean_label(self, label, fallback):
        s = safe_string(label, fallback).strip()
        return s if s else fallback

    def _read_vector(self, pf, vec_idx):
        vec = np.zeros((self.n_nodes, 3), dtype=np.float64)
        for ni in range(self.n_nodes):
            try:
                nv = pf.node_vector(ni, vec_idx)
                vec[ni, 0] = safe_number(getattr(nv, "x", 0.0), 0.0)
                vec[ni, 1] = safe_number(getattr(nv, "y", 0.0), 0.0)
                vec[ni, 2] = safe_number(getattr(nv, "z", 0.0), 0.0)
            except:
                pass
        return np.nan_to_num(vec, nan=0.0, posinf=0.0, neginf=0.0)

    def _bbox_diag(self, xyz):
        try:
            ...
            d = maxs - mins
            return float(np.sqrt(float(d[0] * d[0] + d[1] * d[1] + d[2] * d[2])))
        except Exception:
            return 0.0

    def _canonicalize_displacement_vector(self, vec, state_key=None):
        """Normalize displacement-like vectors to true displacement (u).

        Some T16 outputs expose deformed coordinates (x') in the displacement slot.
        If used directly as u, the mesh becomes over-deformed (x + x').
        We detect this by comparing bbox expansion under two hypotheses:
        H1: vec is displacement -> deformed = x + vec
        H2: vec is absolute coords -> deformed = vec (thus u = vec - x)
        """
        try:
        ...
        if self.nodes is None:
            return arr
        try:
            base = np.asarray(self.nodes, dtype=np.float64)
        except Exception:
            return arr
        if base.ndim != 2 or base.shape[1] < 3 or base.shape[0] != arr.shape[0]:
            return arr
        base = np.nan_to_num(base[:, :3], nan=0.0, posinf=0.0, neginf=0.0)

        b0 = self._bbox_diag(base)
        if (not np.isfinite(b0)) or b0 <= 1e-20:
            return arr

        # Hypothesis H1: vec is displacement.
        b_if_disp = self._bbox_diag(base + arr) / b0
        # Hypothesis H2: vec is absolute deformed coordinates.
        b_if_abs = self._bbox_diag(arr) / b0

        if not np.isfinite(b_if_disp):
            b_if_disp = 1e9
        if not np.isfinite(b_if_abs):
            b_if_abs = 1e9

        score_disp = abs(b_if_disp - 1.0)
        score_abs = abs(b_if_abs - 1.0)
        looks_abs = (b_if_disp > 1.35 and (score_abs + 1e-12) < (score_disp + 1e-12))

        if looks_abs:
            if state_key:
                self.debug_info.append(
                    "Displacement normalization for {}: interpreted as absolute coords "
                    "(bbox_if_disp={:.3f}, bbox_if_abs={:.3f})".format(state_key, b_if_disp, b_if_abs)
                )
            else:
                self.debug_info.append(
                    "Displacement normalization: interpreted as absolute coords "
                    "(bbox_if_disp={:.3f}, bbox_if_abs={:.3f})".format(b_if_disp, b_if_abs)
                )
            return arr - base
        return arr

    def _read_node_scalar(self, pf, scalar_idx):
        # Set linear extrapolation (matches Mentat default: Linear extrapolation, Nodal Averaging off)
        try:
            if hasattr(pf, 'extrapolation'):
                pf.extrapolation('linear')
        except:
            pass
        vals = np.zeros(self.n_nodes, dtype=np.float64)
        for ni in range(self.n_nodes):
            try:
                vals[ni] = safe_number(pf.node_scalar(ni, scalar_idx), 0.0)
            except:
                vals[ni] = 0.0
        return np.nan_to_num(vals, nan=0.0, posinf=0.0, neginf=0.0)

    def _read_elem_scalar_avg_ip(self, pf, scalar_idx):
        """Read element scalar values using linear extrapolation (matches Mentat default).
        
        Marc Mentat default: Extrapolation Method = Linear, Nodal Averaging = off
        
        Linear extrapolation: uses element shape functions to extrapolate from
        integration points to element nodes. The extrapolated node values are then
        averaged per element to get the centroid value.
        
        With py_post.extrapolation('linear'), element_scalar() returns values
        at element nodes (not integration points). We average these for centroid.
        
        Fallback: if extrapolation setting fails, use raw IP averaging.
        """
        # Try to set linear extrapolation (matches Mentat default)
        extrap_set = False
        try:
            if hasattr(pf, 'extrapolation'):
                pf.extrapolation('linear')
        ...
        vals = np.zeros(self.n_elements, dtype=np.float64)
        for out_idx, elem in enumerate(self.elements):
            try:
                src_ei = safe_number(elem.get("orig_index", out_idx), out_idx, as_int=True)
                ip_values = pf.element_scalar(src_ei, scalar_idx)
                vals[out_idx] = self._ip_average(ip_values)
            except:
                vals[out_idx] = 0.0
        return np.nan_to_num(vals, nan=0.0, posinf=0.0, neginf=0.0)

    def _build_output_catalog(self, progress_callback=None, base_pct=88.0, span_pct=12.0):
        self._vector_catalog = {}
        self._node_scalar_catalog = {}
        self._elem_scalar_catalog = {}
        self._var_info = {}
        ...
        if not self._all_increment_indices:
            return

        pf = self.file
        probe_inc = self._all_increment_indices[-1]
        try:
            pf.moveto(probe_inc)
        except:
            pass
        self._emit_progress(progress_callback, base_pct, "Reading output catalog...")

        first_vector_name = None
        n_vectors = safe_number(pf.node_vectors(), 0, as_int=True)
        for nvi in range(n_vectors):
            label = self._clean_label(pf.node_vector_label(nvi), "Node_Vector_{}".format(nvi))
            self._vector_catalog[label] = nvi
            self._var_info[label] = "Vector"
            self._var_locations[label] = "node"
            if first_vector_name is None:
                first_vector_name = label
            if self._disp_vector_name is None and "disp" in label.lower():
                self._disp_vector_name = label
            if n_vectors > 0 and (nvi == n_vectors - 1 or nvi % max(1, n_vectors // 4) == 0):
                self._emit_progress(progress_callback, base_pct + span_pct * 0.30 * float(nvi + 1) / float(n_vectors),
                                    "Reading output catalog (vectors)...")

        n_scalars = safe_number(pf.node_scalars(), 0, as_int=True)
        for nsi in range(n_scalars):
            label = self._clean_label(pf.node_scalar_label(nsi), "Node_Scalar_{}".format(nsi))
            self._node_scalar_catalog[label] = nsi
            self._var_info[label] = "Scalar"
            self._var_locations[label] = "node"
            if n_scalars > 0 and (nsi == n_scalars - 1 or nsi % max(1, n_scalars // 4) == 0):
                self._emit_progress(progress_callback, base_pct + span_pct * (0.30 + 0.30 * float(nsi + 1) / float(n_scalars)),
                                    "Reading output catalog (node scalars)...")

        n_elem_scalars = safe_number(pf.element_scalars(), 0, as_int=True)
        for esi in range(n_elem_scalars):
            label = self._clean_label(pf.element_scalar_label(esi), "Element_Scalar_{}".format(esi))
            self._elem_scalar_catalog[label] = esi
            self._var_info[label] = "Scalar"
            self._var_locations[label] = "element"
            if n_elem_scalars > 0 and (esi == n_elem_scalars - 1 or esi % max(1, n_elem_scalars // 4) == 0):
                self._emit_progress(progress_callback, base_pct + span_pct * (0.60 + 0.40 * float(esi + 1) / float(n_elem_scalars)),
                                    "Reading output catalog (element scalars)...")

        if self._disp_vector_name is None and first_vector_name is not None:
            self._disp_vector_name = first_vector_name

        if self._disp_vector_name is not None:
            self._var_info["Displacement"] = "Vector"
            self._var_locations["Displacement"] = "node"
            self._var_info["Displacement_Magnitude"] = "Scalar"
            self._var_locations["Displacement_Magnitude"] = "node"
        self._emit_progress(progress_callback, base_pct + span_pct, "Output catalog ready")

    def _read_increment_id(self, pf, inc_idx, fallback=None):
        fallback_val = inc_idx if fallback is None else fallback
        try:
            pf.moveto(inc_idx)
        except Exception:
            return safe_number(fallback_val, inc_idx, as_int=True)

        raw = getattr(pf, "increment", None)
        try:
            if callable(raw):
                raw = raw()
        except Exception:
            raw = None
        return safe_number(raw, fallback_val, as_int=True)

    def _probe_increment_info(self, pf, inc_idx, fallback=None):
        fallback_val = inc_idx if fallback is None else fallback
        try:
            pf.moveto(inc_idx)
        except Exception:
        ...
        raw_inc = getattr(pf, "increment", None)
        try:
            if callable(raw_inc):
                raw_inc = raw_inc()
        except Exception:
            raw_inc = None
        inc_id = safe_number(raw_inc, fallback_val, as_int=True)

        raw_title = getattr(pf, "title", None)
        try:
        ...
        if raw_title is None:
            try:
                ...
                raw_title = ""
        title = safe_string(raw_title, "").strip()
        return inc_id, title

    def _reset_increment_classification(self):
        self._increment_ids = {}
        self._increment_labels = {}
        self._static_increment_indices = list(self._all_increment_indices)
        self._harmonic_increment_indices = []
        self._has_harmonic_increments = False
        self._harmonic_base_increment_id = None

    def _classify_increment_types_fast(self, progress_callback=None, base_pct=69.0, span_pct=8.0):
        self._reset_increment_classification()
        all_incs = list(self._all_increment_indices)
        if not all_incs:
            return

        pf = self.file
        self._emit_progress(progress_callback, base_pct, "Detecting increment types...")
        increment_titles = {}
        ids_seq = []

        for pos, inc_idx in enumerate(all_incs):
            inc_id, title = self._probe_increment_info(pf, inc_idx, inc_idx)
            increment_titles[inc_idx] = title
            ids_seq.append(int(inc_id))
            self._increment_ids[inc_idx] = int(inc_id)
            self._increment_labels[inc_idx] = str(int(inc_id))

            if pos == len(all_incs) - 1 or pos % max(1, len(all_incs) // 6) == 0:
                frac = float(pos + 1) / float(max(1, len(all_incs)))
                self._emit_progress(
                    progress_callback,
                    base_pct + span_pct * frac,
                    "Detecting increment types ({}/{})...".format(pos + 1, len(all_incs))
                )

        static_indices = list(all_incs)
        harmonic_indices = []
        has_harmonics = False

        if len(ids_seq) > 1:
            last_id = int(ids_seq[-1])
            run_start = len(ids_seq) - 1
            while run_start > 0 and int(ids_seq[run_start - 1]) == last_id:
                run_start -= 1
            run_len = len(ids_seq) - run_start
            if run_len > 1:
                static_indices = all_incs[:run_start + 1]
                harmonic_indices = all_incs[run_start + 1:]
                if harmonic_indices:
                    has_harmonics = True
                    self._harmonic_base_increment_id = int(last_id)
                    for hi, h_idx in enumerate(harmonic_indices):
                        self._increment_ids[h_idx] = int(last_id)
                        title = safe_string(increment_titles.get(h_idx, ""), "")
                        suffix = None
                        if title:
                            parts = title.replace(",", " ").split()
                            if parts:
                                mt = HARMONIC_TOKEN_RE.match(parts[-1].strip())
                                if not mt:
                                    mt = HARMONIC_TITLE_RE.search(title)
                                if mt:
                                    parsed_base = safe_number(mt.group(1), last_id, as_int=True)
                                    parsed_sfx = safe_number(mt.group(2), hi + 1, as_int=True)
                                    if parsed_base == last_id and parsed_sfx > 0:
                                        suffix = parsed_sfx
                        if suffix is None:
                            suffix = hi + 1
                        self._increment_labels[h_idx] = "{}:{}".format(last_id, suffix)

        if not static_indices and all_incs:
            static_indices = [all_incs[-1]]

        self._static_increment_indices = list(static_indices)
        self._harmonic_increment_indices = list(harmonic_indices) if has_harmonics else []
        self._has_harmonic_increments = bool(has_harmonics and len(self._harmonic_increment_indices) > 0)
        if not self._has_harmonic_increments:
            self._harmonic_base_increment_id = None

        if self._has_harmonic_increments:
            self.debug_info.append(
                "Increment split detected: static={}, harmonic={}, base={}".format(
                    len(static_indices), len(harmonic_indices), self._harmonic_base_increment_id
                )
            )
        else:
            self.debug_info.append("Increment split detected: static={}, harmonic=0".format(len(static_indices)))

        self._emit_progress(progress_callback, base_pct + span_pct, "Increment types detected")

    def _infer_connectivity_index_mode(self, sample_limit=5000):
        total = 0
        id_hits = 0
        one_based_hits = 0
        zero_based_hits = 0

        if self.n_nodes <= 0 or not self.elements:
            self._connectivity_index_mode = False
            return

        for elem in self.elements:
            conn_ids = elem.get("connectivity_ids", []) if isinstance(elem, dict) else []
            for nid in conn_ids:
                nid_i = safe_number(nid, -1, as_int=True)
                total += 1
                if nid_i in self.node_id_to_index:
                    id_hits += 1
                if 1 <= nid_i <= self.n_nodes:
                    one_based_hits += 1
                if 0 <= nid_i < self.n_nodes:
                    zero_based_hits += 1
        ...
        id_ratio = float(id_hits) / float(total)
        idx1_ratio = float(one_based_hits) / float(total)
        idx0_ratio = float(zero_based_hits) / float(total)

        # Index mode if ID-map hits are weak and index-like values dominate.
        self._connectivity_index_mode = bool(id_ratio < 0.50 and max(idx1_ratio, idx0_ratio) >= 0.90)
        if self._connectivity_index_mode:
            mode_name = "1-based" if idx1_ratio >= idx0_ratio else "0-based"
            self.debug_info.append(
                "Connectivity mode detected: {} indices (id_ratio={:.3f}, idx1_ratio={:.3f}, idx0_ratio={:.3f})".format(
                    mode_name, id_ratio, idx1_ratio, idx0_ratio
                )
            )
        else:
            self.debug_info.append(
                "Connectivity mode detected: node IDs (id_ratio={:.3f}, idx1_ratio={:.3f}, idx0_ratio={:.3f})".format(
                    id_ratio, idx1_ratio, idx0_ratio
                )
            )

    def _read_all(self, progress_callback=None):
        pf = self.file
        self.request_number = os.path.splitext(os.path.basename(self.filepath))[0]
        self._emit_progress(progress_callback, 5.0, "Reading geometry...")
        ...
        try:
            ...
            self.n_nodes = safe_number(pf.nodes(), 0, as_int=True)
            self.nodes = np.zeros((self.n_nodes, 3), dtype=np.float64)
            self.node_ids = np.zeros(self.n_nodes, dtype=np.int64)
            self.node_id_to_index = {}
            duplicate_node_ids = 0
            node_step = max(1, self.n_nodes // 50) if self.n_nodes > 0 else 1

            for ni in range(self.n_nodes):
                try:
                    n = pf.node(ni)
                    nid = safe_number(pf.node_id(ni), ni + 1, as_int=True)
                    self.nodes[ni, 0] = safe_number(getattr(n, "x", 0.0), 0.0)
                    self.nodes[ni, 1] = safe_number(getattr(n, "y", 0.0), 0.0)
                    self.nodes[ni, 2] = safe_number(getattr(n, "z", 0.0), 0.0)
                    self.node_ids[ni] = nid
                    nid_i = int(nid)
                    if nid_i in self.node_id_to_index:
                        duplicate_node_ids += 1
                    else:
                        self.node_id_to_index[nid_i] = ni
                except:
                    self.node_ids[ni] = ni + 1
                    fallback_nid = ni + 1
                    if fallback_nid in self.node_id_to_index:
                        duplicate_node_ids += 1
                    else:
                        self.node_id_to_index[fallback_nid] = ni
                if ni == self.n_nodes - 1 or ni % node_step == 0:
                    frac = float(ni + 1) / float(max(1, self.n_nodes))
                    self._emit_progress(progress_callback, 8.0 + 22.0 * frac,
                                        "Reading nodes ({}/{})...".format(ni + 1, self.n_nodes))

            raw_elem_count = safe_number(pf.elements(), 0, as_int=True)
            self.elements = []
            elem_step = max(1, raw_elem_count // 50) if raw_elem_count > 0 else 1
            for ei in range(raw_elem_count):
                try:
                    elem = pf.element(ei)
                    elem_id = safe_number(pf.element_id(ei), ei + 1, as_int=True)
                    elem_type = safe_number(getattr(elem, "type", 0), 0, as_int=True)
                    raw_conn = list(getattr(elem, "items", []))
                    conn = self._normalize_connectivity(elem_type, raw_conn)
                    if len(conn) < 3:
                        continue
                    self.elements.append({
                        "id": int(elem_id),
                        "type": int(elem_type),
                        "connectivity_ids": conn,
                        "orig_index": ei
                    })
                except:
                    pass
                if ei == raw_elem_count - 1 or ei % elem_step == 0:
                    frac = float(ei + 1) / float(max(1, raw_elem_count))
                    self._emit_progress(progress_callback, 30.0 + 35.0 * frac,
                                        "Reading elements ({}/{})...".format(ei + 1, raw_elem_count))

            self.n_elements = len(self.elements)
            self._infer_connectivity_index_mode()
            self.nodes = np.nan_to_num(self.nodes, nan=0.0, posinf=0.0, neginf=0.0)
            if self.node_ids is not None and len(self.node_ids) > 0:
                ...
                if duplicate_node_ids > 0:
                    self.debug_info.append("WARNING: duplicate node IDs detected: {}".format(duplicate_node_ids))
        except Exception as ex:
            self.debug_info.append("Geometry error: {}".format(str(ex)))

        # Build state metadata quickly (without full field extraction)
        try:
            self._emit_progress(progress_callback, 67.0, "Reading increment list...")
            total_increments = safe_number(pf.increments(), 0, as_int=True)
            start_inc = 1 if total_increments > 1 else 0
            self._all_increment_indices = list(range(start_inc, total_increments))
            if not self._all_increment_indices and total_increments > 0:
                self._all_increment_indices = [0]
            self._time_denominator = float(max(1, total_increments - 1))
            self._classify_increment_types_fast(progress_callback=progress_callback, base_pct=69.0, span_pct=8.0)

            inc_total = len(self._all_increment_indices)
            inc_step = max(1, inc_total // 30) if inc_total > 0 else 1
            for pos, _inc in enumerate(self._all_increment_indices):
                if pos + 1 == inc_total or (pos + 1) % inc_step == 0:
                    frac = float(pos + 1) / float(max(1, inc_total))
                    self._emit_progress(progress_callback, 70.0 + 15.0 * frac,
                                        "Reading increments ({}/{})...".format(pos + 1, inc_total))

            self._active_increment_indices = list(self._all_increment_indices)
            self._rebuild_states_from_active()

            self._build_output_catalog(progress_callback=progress_callback, base_pct=86.0, span_pct=14.0)
        except Exception as ex:
            self.debug_info.append("States error: {}".format(str(ex)))
        self._emit_progress(progress_callback, 100.0, ".t16/.t19 file loaded")

    def _rebuild_states_from_active(self):
        self.states = {}
        for inc_idx in self._active_increment_indices:
            state_key = self._build_unique_state_key(inc_idx)
            inc_label = self._increment_labels.get(inc_idx)
            if inc_label is None:
                inc_value = int(inc_idx)
            elif ":" in inc_label:
                inc_value = inc_label
            else:
                inc_value = safe_number(self._increment_ids.get(inc_idx, inc_idx), inc_idx, as_int=True)
            self.states[state_key] = {
                "time": float(inc_idx) / float(max(1.0, self._time_denominator)),
                "increment": inc_value,
                "variables": {}
            }
        self._loaded_outputs = set()

    def get_available_increments(self):
        return list(self._all_increment_indices)

    def get_increment_partitions(self):
        return {
            "all": list(self._all_increment_indices),
            "static": list(self._static_increment_indices),
            "harmonic": list(self._harmonic_increment_indices),
            "has_harmonics": bool(self._has_harmonic_increments),
            "harmonic_increment": self._harmonic_base_increment_id,
            "labels": dict(self._increment_labels),
            "ids": dict(self._increment_ids)
        }

    def get_harmonic_base_displacement(self):
        """Read displacement at the last static increment (harmonic base).

        In Marc harmonic analysis on pre-stressed structures, the harmonic
        sub-increments store TOTAL displacement (u_static + u_harmonic).
        This method returns u_static so callers can subtract it to isolate
        just the harmonic perturbation.
        """
        if not self._has_harmonic_increments:
            return None
        if not self._static_increment_indices:
            return None
        if self.file is None or self.nodes is None:
            return None
        if self._disp_vector_name is None:
            return None
        if self._disp_vector_name not in self._vector_catalog:
            return None

        base_inc_idx = self._static_increment_indices[-1]
        pf = self.file
        try:
            pf.moveto(base_inc_idx)
            disp_vec = self._read_vector(pf, self._vector_catalog[self._disp_vector_name])
            disp_vec = self._canonicalize_displacement_vector(disp_vec, state_key="harmonic_base")
            return np.nan_to_num(disp_vec, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float64)
        except Exception:
            return None

    def set_active_increments_for_mode(self, mode="static"):
        mode_l = safe_string(mode, "static").strip().lower()
        if mode_l == "harmonic":
            selected = list(self._harmonic_increment_indices)
        else:
            selected = list(self._static_increment_indices)
            if not selected:
                selected = list(self._all_increment_indices)

        if selected != self._active_increment_indices:
            self._active_increment_indices = list(selected)
            self._rebuild_states_from_active()
        return list(self._active_increment_indices)

    def set_active_increment_selection(self, mode="single", single_increment=None, range_start=None, range_end=None, step=1):
        all_incs = list(self._all_increment_indices)
        if not all_incs:
            self._active_increment_indices = []
            self._rebuild_states_from_active()
        ...
        mode = safe_string(mode, "single").lower()
        step_i = safe_number(step, 1, as_int=True)
        if step_i < 1:
            step_i = 1
        ...
        if mode == "all":
            ...
            s = safe_number(range_start, all_incs[0], as_int=True)
            e = safe_number(range_end, all_incs[-1], as_int=True)
            if s > e:
                s, e = e, s
            selected = [inc for inc in all_incs if inc >= s and inc <= e and ((inc - s) % step_i == 0)]
        else:
            inc = safe_number(single_increment, all_incs[-1], as_int=True)
    ...
    def load_selected_outputs(self, selected_outputs=None, progress_callback=None):
        if self.file is None:
            return

        requested = set(selected_outputs or [])
        requested.add("Displacement")
        if "Displacement_Magnitude" in requested:
            requested.add("Displacement")

        missing = [name for name in requested if name not in self._loaded_outputs]
        if not missing:
            return

        pf = self.file
        total_states = len(self._active_increment_indices)
        if total_states == 0:
            return

        for si, inc_idx in enumerate(self._active_increment_indices):
            try:
                pf.moveto(inc_idx)
            except:
                pass
            ...
            if state is None:
                continue
            vars_dict = state.setdefault("variables", {})

            # Deformed mesh in the viewer always depends on canonical Displacement.
            if ("Displacement" in requested or "Displacement_Magnitude" in requested) and "Displacement" not in vars_dict:
                if self._disp_vector_name in self._vector_catalog:
                    disp_vec = self._read_vector(pf, self._vector_catalog[self._disp_vector_name])
                    disp_vec = self._canonicalize_displacement_vector(disp_vec, state_key=state_key)
                    vars_dict["Displacement"] = disp_vec
                    if self._disp_vector_name not in vars_dict:
                        vars_dict[self._disp_vector_name] = disp_vec
                    vars_dict["Displacement_Magnitude"] = np.sqrt(np.nansum(disp_vec ** 2, axis=1))

            for out_name in missing:
                if out_name in ("Displacement", "Displacement_Magnitude"):
                    continue
                if out_name in vars_dict:
                    continue
                if out_name in self._vector_catalog:
                    vars_dict[out_name] = self._read_vector(pf, self._vector_catalog[out_name])
                elif out_name in self._node_scalar_catalog:
                    vars_dict[out_name] = self._read_node_scalar(pf, self._node_scalar_catalog[out_name])
                elif out_name in self._elem_scalar_catalog:
                    vars_dict[out_name] = self._read_elem_scalar_avg_ip(pf, self._elem_scalar_catalog[out_name])

            if progress_callback:
                try:
                    pct = int((100.0 * (si + 1)) / float(total_states))
                    progress_callback(pct, "Reading selected T16 outputs ({}/{})...".format(si + 1, total_states))
                except:
                    pass

        self._loaded_outputs.update(missing)
        if "Displacement" in requested:
            self._loaded_outputs.add("Displacement")
            self._loaded_outputs.add("Displacement_Magnitude")

    def _resolve_node_index(self, nid):
        try:
            nid_i = safe_number(nid, -1, as_int=True)
        except Exception:
            return None
        ...
        if nid_i in self.node_id_to_index:
            return int(self.node_id_to_index[nid_i])

        if self._connectivity_index_mode:
            if nid_i > 0 and nid_i <= self.n_nodes:
                return int(nid_i - 1)
            if 0 <= nid_i < self.n_nodes:
                return int(nid_i)
            return None

        # Fallbacks are allowed only when they exactly match the known node ID table.
        if self.node_ids is not None and len(self.node_ids) == self.n_nodes:
            if nid_i > 0 and (nid_i - 1) < self.n_nodes:
                try:
                    if int(self.node_ids[nid_i - 1]) == nid_i:
                        return int(nid_i - 1)
                except Exception:
                    pass
            if 0 <= nid_i < self.n_nodes:
                try:
                    if int(self.node_ids[nid_i]) == nid_i:
                        return int(nid_i)
                except Exception:
                    pass
    ...
    def get_connectivity_as_indices(self, elem):
        ...
        for nid in conn_ids:
            nid_i = safe_number(nid, -1, as_int=True)
            parsed_ids.append(nid_i)
            idx = self._resolve_node_index(nid_i)
            if idx is None:
                unresolved = True
                break
            indices.append(idx)
        ...
        # Per-element fallback: accept pure index-like connectivity when ID mapping misses.
        if self.n_nodes > 0 and parsed_ids:
            if all(1 <= v <= self.n_nodes for v in parsed_ids):
                return [int(v - 1) for v in parsed_ids]
            if all(0 <= v < self.n_nodes for v in parsed_ids):
                return [int(v) for v in parsed_ids]
    ...
    def get_nodes(self, state_name=None, scale=1.0, var_name="Displacement"):
        ...
        if state_name and state_name in self.states:
            ...
            if disp is None and var_name not in self._loaded_outputs:
                try:
                    self.load_selected_outputs([var_name])
                    disp = self.states[state_name]["variables"].get(var_name)
                except:
                    disp = None
            if disp is not None:
    ...
    def get_values(self, state_name, var_name):
        ...
        if vals is None and var_name not in self._loaded_outputs:
            try:
                self.load_selected_outputs([var_name])
                vals = self.states[state_name]["variables"].get(var_name)
            except:
    ...
    def get_element_values(self, state_name, var_name):
        ...
        try:
            ...
            if len(result) == n_nodes and self.n_elements > 0:
    ...
    def get_variable_locations(self):
        if self._var_locations:
            return dict(self._var_locations)
        locations = {}
        n_nodes = len(self.nodes) if self.nodes is not None else 0
        for st in self.states.values():
            for var_name, var_data in st.get("variables", {}).items():
...
def generate_html(reader, progress_callback=None, selected_output=None, viewer_mode="static", export_centroid=True, export_all_edges=False):
    ...
    if original_nodes is None:
        raise ValueError("No nodes found in .t16/.t19 file")
    ...
    # =========================================================================
    html = '''<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>T16 3D Viewer - Vibracoustic</title>
<style>
...
.measure-node-label{position:absolute;padding:2px 8px;border-radius:999px;color:#fff;font:700 11px/1 Arial,sans-serif;box-shadow:0 1px 4px rgba(0,0,0,0.35);border:1px solid rgba(255,255,255,0.72);white-space:nowrap;transform:translate(-50%,-50%)}
...
<div style="display:flex;justify-content:space-between;align-items:center;gap:6px;margin-top:6px"><span style="font-size:10px;font-weight:bold;color:#6A1B9A">Extrapolation</span><button class="leg-btn leg-btn-highlight" onclick="openExtrapolationDialog()">Visualization Options</button></div>
...
function isElementDisplayMode(){
return centroidMode||isElementLocalContourMode();
}
...
ci.textContent='(extrapolated nodal values)';
}
function getExtrapolationSummaryText(){
var tail='Extrapolated nodal contour';
...
var preset=getPendingExtrapolationStandardPreset();
if(!isElementBasedVarForExtrapolation(currentVar)){
var txt='The current variable is already displayed from extrapolated nodal values in this T16 export. Mentat extrapolation controls are shown for parity, but the visible contour already comes from nodal values.';
...
var txt3='Marc Mentat offers Linear, Translate and Average together with Nodal Averaging On/Off. In this T16 export, Nodal Averaging Off uses the stored per-element values to show an element-local contour.';
...
var txt4='Marc Mentat offers Linear, Translate and Average together with Nodal Averaging On/Off. With this T16 HTML payload, the strongest visible change is the Nodal Averaging toggle; Linear, Translate and Average can become visually equivalent without corner or integration-point data in the browser.';
...
var prevElementDisplay=isElementDisplayMode();
...
if(isElementDisplayMode()&&centroidRawColors){
...
if(!data||!data.items||data.items.length===0||!isElementDisplayMode())return;
...
if(isElem||isElementDisplayMode()){
...
if(isElementDisplayMode()&&!isElem)isElem=true;
if(!isElementDisplayMode()&&p0!=='E'&&p0!=='N'){
...
var mnKey=isElementDisplayMode()?'centroid_min':'color_min';
var mxKey=isElementDisplayMode()?'centroid_max':'color_max';
...
if(csd){
if(isElementDisplayMode()){dataMin=csd.centroid_min;dataMax=csd.centroid_max;}
else{dataMin=csd.color_min;dataMax=csd.color_max;}
if(isElementDisplayMode()&&centroidRawColors){cm(getRenderNodes(),rawColors);}
...
N_DISC=12;
legFontSize=14;
applyExtrapolationSettings('linear','off',{silent:true,keepDialog:true});
updateLegendFormatControls();
...
vrfSig=lo+':'+hi+':'+(centroidMode?'centroid':(isElementLocalContourMode()?'elementlocal':'nodal'))+':'+(currentVar||'');
}
return [
renderMode,
'edge='+edgeMode,
...
curColors=colors;
var cuts=getActiveCuts();
var useCentroid=isElementDisplayMode()&&centroidRawColors&&!noContour;
...
if(isElementDisplayMode()&&sd.centroid_min!==undefined){
...
if(!isElementDisplayMode()&&!noContour&&!dynamicLegend&&rawColors&&(Math.abs(curMin-dataMin)>1e-20||Math.abs(curMax-dataMax)>1e-20)){
...
if(hasCustomLegend())legendCustomValues=buildLinearLegendValues(curMin,curMax);
updateLegendRangeInputs();
if(!legendVisibilityRebuildGuard&&!isElementDisplayMode()&&!noContour&&rawColors){
...
var drawColors=null;
if(cst&&AD[cst]){
if(isElementDisplayMode()&&centroidRawColors){drawColors=rawColors;}
...
if(cfg.extrapolationMethod!==undefined||cfg.extrapolationNodalAveraging!==undefined){
...
try{
console.log('[T16] Build='+BUILD_REV);
var fLen=(Array.isArray(F)?F.length:'lazy');
var femLen=(Array.isArray(FEM)?FEM.length:'lazy');
console.log('[T16] Data check: ON='+ON.length+' F='+fLen+' BF='+BF.length+' BFE='+BFE.length+' FEM='+femLen+' SL='+SL.length);
console.log('[T16] B='+B+' CT='+JSON.stringify(CT)+' IS="'+IS+'" currentVar="'+currentVar+'"');
console.log('[T16] Output vars='+Object.keys(OUT_STATE_INDEX).join(','));
...
init();
console.log('[T16] Init complete');
setTimeout(function(){
if(loadEmbeddedConfigIfAny()){
console.log('[T16] Embedded runtime config restored');
}
},60);
}catch(initErr){
console.error('[T16] Init failed:',initErr);
...
class App:
    ...
    def setup(self):
        ...
        self.export_all_edges_cb = tk.Checkbutton(
            options_row,
            text="Enable All Edges option in HTML (EXTREMELY HEAVY)",
    ...
    def browse(self):
        f = filedialog.askopenfilename(filetypes=POST_FILE_DIALOG_TYPES)
        if f:
            ...
            if loaded_path and new_path != loaded_path:
                self.reset_session_state("Previous session cleared. Loading new T16...")
    ...
    def on_file_path_changed(self, *args):
        ...
        if path and os.path.exists(path):
            ...
            if loaded_path and new_path != loaded_path:
                self.reset_session_state("Previous session cleared. Loading new T16...")
            self.root.after(100, lambda p=new_path: self.load_t16(p))
    ...
    def apply_mode_rules(self):
        ...
        base_mode_text = "Static: normal workflow | Harmonic: External Surface + Displacement only"
        if has_reader and self.reader is not None:
            try:
                parts = self.reader.get_increment_partitions()
            except Exception:
                parts = {}
            static_count = len(parts.get("static", []) or [])
            harmonic_count = len(parts.get("harmonic", []) or [])
            has_harmonic_increments = harmonic_count > 0

            if hasattr(self, 'mode_static_rb') and self.mode_static_rb:
                self.mode_static_rb.config(state='normal')
            if hasattr(self, 'mode_harmonic_rb') and self.mode_harmonic_rb:
                self.mode_harmonic_rb.config(state='normal' if has_harmonic_increments else 'disabled')

            if (not has_harmonic_increments) and mode == "harmonic":
                self.viewer_mode.set("static")
                mode = "static"

            self.mode_info_label.config(
                text=base_mode_text + " | Detected increments: Static={} | Harmonic={}".format(
                    static_count, harmonic_count
                ),
                fg='#666'
            )
        else:
            self.set_mode_controls_enabled(False)
            self.mode_info_label.config(text=base_mode_text, fg='#666')
            has_harmonic_increments = False
        ...
        if mode == "harmonic":
            if not has_harmonic_increments:
                if hasattr(self, 'export_all_edges_cb') and self.export_all_edges_cb:
                ...
                self.output_info_label.config(
                    text="No harmonic increments detected in this file.",
                    fg='red'
                )
                self.st.set("Harmonic mode unavailable: no harmonic increments detected")
                return

            # Harmonic mode exports only Displacement; extra element data stays disabled.
            if hasattr(self, 'export_all_edges_cb') and self.export_all_edges_cb:
                self.export_all_edges_cb.config(state='normal')
            self.output_listbox.config(state='disabled')
    ...
    def load_t16(self, path):
        if not os.path.exists(path):
            return
        
        self.st.set("Auto-loading .t16/.t19 file...")
        self.root.update()
        
        try:
            self._close_reader()
            
            self.reader = T16Reader(path).open()
            ...
            info_msg = "Loaded: {:,} nodes, {:,} elements, {} states (Static={}, Harmonic={}), {} outputs".format(
                self.reader.n_nodes, self.reader.n_elements,
                len(self.reader.states), static_count, harmonic_count, len(self.available_outputs))
            self.st.set(info_msg)
            
        except Exception as e:
            import traceback
            self.st.set("Error loading T16: {}".format(str(e)))
            messagebox.showerror("Error loading T16", str(e))
            traceback.print_exc()
    ...
    def generate(self):
        if not self.reader:
            messagebox.showwarning("Error", "Please select a .t16/.t19 file first")
            return
        ...
        if mode == "harmonic":
            harmonic_count = len(parts.get("harmonic", []) or [])
            if harmonic_count <= 0:
                messagebox.showwarning("Error", "No harmonic increments were detected in this .t16/.t19 file.")
                return

        active_increments = self.reader.set_active_increments_for_mode(mode)
        if not active_increments:
            messagebox.showwarning("Error", "No increments available for {} mode.".format(mode.capitalize()))
            return

        start_ts = time.perf_counter()
        self.update_progress(
            0,
            "Preparing selected outputs ({}) : {} | Increments: {} | Element Data: {} | All Edges Opt: {} | Cache: Auto".format(
                mode.capitalize(),
                selected_output,
                len(active_increments),
                "On" if export_centroid else "Off",
                "On" if export_all_edges else "Off"
            )
        )
        
        try:
            def preload_progress(pct, msg):
                self.update_progress(5 + (pct * 0.20), msg)

            def html_progress(pct, msg):
                self.update_progress(25 + (pct * 0.75), msg)

            self.reader.load_selected_outputs([selected_output], progress_callback=preload_progress)
            ...
            message += "Exported increments: {}\n".format(len(active_increments))
