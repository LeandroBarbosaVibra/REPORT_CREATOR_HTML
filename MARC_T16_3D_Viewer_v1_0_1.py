...
NEW: Element centroid = average of linearly extrapolated node values
...
<div class="range-row"><span>Start Inc:</span><input type="range" id="gif-start" min="0" max="''' + str(max(0, n_states - 1)) + '''" value="0" step="1" oninput="ugrl()"><span class="rv" id="gif-start-val">0</span></div>
<div class="range-row"><span>End Inc:</span><input type="range" id="gif-end" min="0" max="''' + str(max(0, n_states - 1)) + '''" value="''' + str(max(0, n_states - 1)) + '''" step="1" oninput="ugrl()"><span class="rv" id="gif-end-val">''' + str(max(0, n_states - 1)) + '''</span></div>
...
function ugrl(){
const si=parseInt(document.getElementById('gif-start').value);
const ei=parseInt(document.getElementById('gif-end').value);
document.getElementById('gif-start-val').textContent=SL[si]?SL[si].increment:si;
document.getElementById('gif-end-val').textContent=SL[ei]?SL[ei].increment:ei;
}

function getAnimSpeedValue(){
var speed=parseInt(document.getElementById('anim-speed').value,10);
...
cfg.gifStart=document.getElementById('gif-start').value;
cfg.gifEnd=document.getElementById('gif-end').value;
cfg.animSpeed=document.getElementById('anim-speed').value;
cfg.gifScale=document.getElementById('gif-scale').value;
...
// Animation range
if(cfg.gifStart)document.getElementById('gif-start').value=cfg.gifStart;
if(cfg.gifEnd)document.getElementById('gif-end').value=cfg.gifEnd;
if(cfg.animSpeed){document.getElementById('anim-speed').value=cfg.animSpeed;document.getElementById('speed-val').textContent=cfg.animSpeed;}
if(cfg.gifScale){document.getElementById('gif-scale').value=cfg.gifScale;document.getElementById('gif-scale-val').textContent=cfg.gifScale;}
if(VIEWER_MODE==='harmonic'){
...
class App:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("T16 3D Viewer v1.0.1")
    ...
    def apply_mode_rules(self):
        ...
        if mode == "harmonic":
            ...
            # Harmonic mode exports only Displacement; centroid data must stay disabled.
            self.export_centroid_var.set(False)
            if hasattr(self, 'export_centroid_cb') and self.export_centroid_cb:
                self.export_centroid_cb.config(state='disabled')
            if hasattr(self, 'export_all_edges_cb') and self.export_all_edges_cb:
    ...
    def generate(self):
        ...
        if mode == "harmonic":
            # Safety gate: harmonic output is displacement-only.
            self.export_centroid_var.set(False)
        ...
        self.update_progress(
            0,
            "Preparing selected outputs ({}) : {} | Increments: {} | Centroid: {} | All Edges Opt: {} | Cache: Auto".format(
                mode.capitalize(),
                selected_output,
